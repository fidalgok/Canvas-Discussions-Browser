...
export const syncDiscussions = mutation({
    ...
    handler: async (ctx, { apiUrl, apiKey, courseId }) => {
        ...
        if (!topicsResponse.ok) {
            throw new Error(`Failed to fetch discussion topics: ${await topicsResponse.text()}`);
        }
        ...
        for (const topic of topics) {
            ...
            let discussion = await ctx.db.query("discussions")
                .withIndex("by_canvas_discussion_id", q => q.eq("canvas_discussion_id", topic.id))
                .first();
            ...
            const discussionId = (await ctx.db.query("discussions").withIndex("by_canvas_discussion_id", q => q.eq("canvas_discussion_id", topic.id)).first())._id;
            ...
            while (hasMore) {
                ...
                if (!entriesResponse.ok) {
                    console.error(`Failed to fetch entries for topic ${topic.id}: ${await entriesResponse.text()}`);
                    break;
                }
            ...
            for (const entry of allEntries) {
                ...
                let user = await ctx.db.query("users")
                    .withIndex("by_canvas_user_id", q => q.eq("canvas_user_id", entry.user_id))
                    .first();
                if (!user && entry.user_id && !seenUsers.has(entry.user_id)) {
                    seenUsers.add(entry.user_id);
                    const userUrl = `${apiUrl}/users/${entry.user_id}`;
                    const userResponse = await fetch(userUrl, { headers: { 'Authorization': `Bearer ${apiKey}` } });
                    if (userResponse.ok) {
                ...
                const userId = (await ctx.db.query("users").withIndex("by_canvas_user_id", q => q.eq("canvas_user_id", entry.user_id)).first())?._id;


                // Store post
                let post = await ctx.db.query("posts")
                    .withIndex("by_canvas_post_id", q => q.eq("canvas_post_id", entry.id))
                    .first();
                ...
                const postId = (await ctx.db.query("posts").withIndex("by_canvas_post_id", q => q.eq("canvas_post_id", entry.id)).first())?._id;
                ...
                for (const reply of replies) {
                    ...
                    let replyUser = await ctx.db.query("users")
                        .withIndex("by_canvas_user_id", q => q.eq("canvas_user_id", reply.user_id))
                        .first();
                    if (!replyUser && reply.user_id && !seenUsers.has(reply.user_id)) {
                        ...
                        const replyUserResponse = await fetch(replyUserUrl, { headers: { 'Authorization': `Bearer ${apiKey}` } });
                    ...
                    const replyUserId = (await ctx.db.query("users").withIndex("by_canvas_user_id", q => q.eq("canvas_user_id", reply.user_id)).first())?._id;

                    // Store reply
                    let convexReply = await ctx.db.query("replies")
                        .withIndex("by_canvas_reply_id", q => q.eq("canvas_reply_id", reply.id))
                        .first();
