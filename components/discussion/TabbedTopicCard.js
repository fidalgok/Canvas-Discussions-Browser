...
export default function TabbedTopicCard({ topic, className = "" }) {
  ...
  console.log("Rendering TabbedTopicCard for topic:", topic);
  ...
  console.log(topic.allStudentsWithStatus);
  const allStudents = (topic.allStudentsWithStatus || []).map((student) => ({
    name: student.name,
  ...
  return (
    <div
      className={`rounded-none border-l-4 mt-6 mb-8 ${className}`}
      style={{
    ...
    >
      ...
      {/* Footer Info */}
      <div
        className="px-6 py-3 mt-4 text-sm"
        style={{
          ...
          <div>
            <span>
              <span className="font-medium">
                {topic.totalStudentPosts} posts • {topic.totalTeacherReplies}{" "}
                replies
              </span>
